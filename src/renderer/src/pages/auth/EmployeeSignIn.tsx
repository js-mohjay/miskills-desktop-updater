...
const SignIn = () => {
  ...
  const sendOtpMutation = useMutation({
    mutationFn: async (identifier: string) => authApi.employeeSendOtp(identifier),
  ...
  const verifyOtpMutation = useMutation({
    mutationFn: async (payload: { identifier: string; otp: string }) =>
      authApi.employeeVerifyOtp(payload.identifier, payload.otp),
    ...
    onSuccess: (res) => {
      ...
      requestAnimationFrame(() => {
        ...
        else if (d?.user?.role === "training") navigate("/training-admin");
        else if(d?.user?.role === "placement") navigate("/placement-admin");
        else if(d?.user?.role === "instructor") navigate("/instructor");
    ...
    <div className="h-screen flex flex-col justify-center items-center">
      ...
      >
        ...
        <motion.p
          ...
          transition={{ duration: 0.5 }}
        >
          Not an Employee? <Link to="/signin" className="text-violet-400 hover:text-violet-500 hover:underline transition duration-300 cursor-pointer">Sign-In as a user.</Link>
        </motion.p>
